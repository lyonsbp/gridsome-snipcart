<template>
  <div class="carousel relative shadow-2xl bg-white">
    <div class="carousel-inner relative overflow-hidden w-full">
      <!-- <template v-for="({ node }, index) in $static.products.edges">
        <input
          :id="`carousel-${index+1}`"
          :key="`input-${node.id}`"
          class="carousel-open"
          ...
          :checked="index === 0"
        />
        <div :key="`item-${node.id}`" class="carousel-item absolute opacity-0" style="height:50vh;">
          <div
        ...
        <label
          :key="`prev-${node.id}`"
          :for="`carousel-${index+1 === 1 ? $static.products.edges.length : index}`"
      ...
      <!--  <ol class="carousel-indicators">
        <template v-for="({ node }, index) in $static.products.edges">
          <li :key="node.id" class="inline-block mr-3">
            <label
